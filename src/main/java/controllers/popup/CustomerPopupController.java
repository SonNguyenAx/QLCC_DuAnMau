...
public class CustomerPopupController {
    ...
    public void edit(CustomerPopupView view, Resident customer, SuccessCallback sc, ErrorCallback ec) {
        if (previousView != null && previousView.isDisplayable()) {
            previousView.requestFocus();
            return;
        }
        ...
        view.getTxtName().setText(customer.getResidentName());
        view.getTxtPhoneNumber().setText(customer.getPhone());
//        view.getTxtAddress().setText(customer.getCCCD());
...
//        c.setPhoneNumber(phoneNumber);
    ...
    public boolean editCustomer(CustomerPopupView view, Resident c) throws Exception {
        String name = view.getTxtName().getText(), address = view.getTxtAddress().getText(), phoneNumber = view.getTxtPhoneNumber().getText();
        if (phoneNumber.isEmpty()) {
            throw new Exception("Vui lòng điền đầy đủ thông tin");
        }
